...
def test_case_sensitivity_blocked_pattern(file_tool, context, tmp_path):
    ...
    if os.name == 'nt':
        # Windows: Must Block
        assert result.status == "error", "Windows should block 'Secrets/' (case-insensitive)"
...
def test_max_path_length_validation(file_tool, context):
    ...
    with patch.object(FileTool, "_write_file", side_effect=OSError(36, "File name too long")):
        result = file_tool.run({
            "operation": "write_file",
            "path": long_name,
            "content": "test"
        ...
        assert "File name too long" in result.error["message"] or "IOError" in result.error["code"]
