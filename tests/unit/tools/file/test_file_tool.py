...
def test_read_file_success(file_tool, context, tmp_path):
    ...
    result = file_tool.run({"operation": "read_file", "path": "test.txt"}, context)
...
def test_read_file_path_traversal(file_tool, context):
    ...
    result = file_tool.run(
        {"operation": "read_file", "path": "../../../etc/passwd"}, context
    )
...
def test_write_file_atomic(file_tool, context, tmp_path):
    ...
    result = file_tool.run(
        {"operation": "write_file", "path": "atomic.txt", "content": "atomic content"},
        context,
    )
...
def test_write_file_blocked_pattern(file_tool, context):
    ...
    result = file_tool.run(
        {"operation": "write_file", "path": ".env", "content": "SECRET=123"}, context
    )
...
def test_list_files(file_tool, context, tmp_path):
    ...
    result = file_tool.run({"operation": "list_files", "path": "."}, context)
...
def test_read_ads_blocked_windows(file_tool, context, tmp_path):
    ...
    result = file_tool.run(
        {"operation": "read_file", "path": "normal.txt:secret"}, context
    )
    ...
    assert (
        "Invalid path" in result.error["message"] or "Security" in result.error["code"]
    )
