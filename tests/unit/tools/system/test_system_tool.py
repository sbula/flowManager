...
def test_system_tool_access_denied_dev(system_tool, dev_context):
    ...
    result = system_tool.run(
        {"operation": "install_package", "package": "curl"}, dev_context
    )
...
def test_install_package_success(system_tool, sre_context):
    ...
    with patch("subprocess.run") as mock_run:
        ...
        result = system_tool.run(
            {"operation": "install_package", "manager": "apt", "package": "curl"},
            sre_context,
        )
...
def test_system_ctl(system_tool, sre_context):
    ...
    with patch("subprocess.run") as mock_run:
        mock_run.return_value.returncode = 0
        result = system_tool.run(
            {"operation": "system_ctl", "service": "docker", "action": "status"},
            sre_context,
        )
...
def test_system_tool_timeout(system_tool, sre_context):
    ...
    with patch("subprocess.run") as mock_run:
        ...
        result = system_tool.run(
            {"operation": "install_package", "manager": "apt", "package": "heavy-pkg"},
            sre_context,
        )
