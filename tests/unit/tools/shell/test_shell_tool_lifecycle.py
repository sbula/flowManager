...
from src.flow.tools.shell.win32_job import WindowsJobObject, JOB_OBJECT_LIMIT_KILL_ON_JOB_CLOSE
...
def test_job_object_creation_on_windows(context):
    ...
    with patch("os.name", "nt"), \
         patch("ctypes.windll.kernel32.CreateJobObjectW", return_value=123) as mock_create, \
         patch("ctypes.windll.kernel32.SetInformationJobObject", return_value=True) as mock_set:
...
def test_process_assignment_to_job(shell_tool, context):
    ...
    with patch("os.name", "nt"), \
         ...
         patch("src.flow.tools.shell.win32_job.WindowsJobObject") as mock_job_cls:
...
def test_child_process_cleanup_on_exception(shell_tool, context):
    ...
    with patch("subprocess.Popen") as mock_popen, \
         patch("src.flow.tools.shell.win32_job.WindowsJobObject"):
...
def test_grandchild_fate_sharing_logic(context):
    ...
    with patch("os.name", "nt"), \
         patch("ctypes.windll.kernel32.CreateJobObjectW", return_value=123), \
         patch("ctypes.windll.kernel32.SetInformationJobObject", return_value=True) as mock_set:
