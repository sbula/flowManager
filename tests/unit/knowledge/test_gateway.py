...
def test_gateway_init(mock_config):
    with patch('workflow_core.knowledge.llm.gateway.GeminiAdapter', return_value=MockProvider()) as MockGemini, \
         patch('workflow_core.knowledge.llm.gateway.AnthropicAdapter', return_value=MockProvider()) as MockAnthropic, \
         patch.dict('os.environ', {"GOOGLE_API_KEY": "fake", "ANTHROPIC_API_KEY": "fake"}):
