...
def test_index_command_invokes_engine():
    ...
    with patch("sys.argv", ["cli.py", "index", "src"]), patch(
        "workflow_core.knowledge.service.KnowledgeService"
    ) as MockService, patch(
        "workflow_core.knowledge.ingestion.engine.IngestionEngine"
    ) as MockEngine, patch(
        "workflow_core.knowledge.ingestion.hasher.ManifestManager"
    ) as MockManifest, patch(
        "json.load", return_value={}
    ):  # json is imported at top level, so we can patch json.load? No, better patch the module where it is used or builtin
...
def test_install_hooks_creates_file(tmp_path):
    ...
    with patch("sys.argv", ["cli.py", "install-hooks"]), patch(
        "pathlib.Path", wraps=tmp_path
    ):  # Trick Path to look at tmp
