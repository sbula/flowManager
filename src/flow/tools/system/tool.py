...
class SystemTool(Tool):
    ...
    def _install_package(self, manager: str, package: str,
        ...
        if manager == "apt":
            ...
            raise ToolError(f"Unsupported manager: {manager}",
                            code="ValidationError")

        return self._run_command(cmd, context)

    def _system_ctl(self, service: str, action: str,
                    context: ToolContext) -> ToolResult:
        if not service or not action:
            raise ToolError("Service and action required",
                            code="ValidationError")
