...
class FileTool(Tool):
        ...
        "properties": {
            "operation": {
                ...
                "enum": ["read_file", "write_file", "list_files",
                         "delete_file", "create_directory", "edit_file",
                         "search_file", "count_matches"]
            },
            "path": {"type": "string"},
            "content": {"type": "string"},
    ...
    def _edit_file(self, path: Path,
                   edits: List[Dict[str, Any]]) -> ToolResult:
        if not path.exists():
            return ToolResult(status="error", error={
                "code": "FileNotFound",
    ...
    def _validate_path(self, path_str: str, context: ToolContext) -> Path:
        ...
        try:
            ...
            if check_path.is_symlink():
                raise ToolError(f"Symlinks are not allowed: {path_str}",
                                code="SecurityError")
            ...
            if os.name == 'nt' and str(target_path).count(':') > 1:
                 raise ToolError(f"Alternate Data Streams are not allowed: {path_str}",
                                 code="SecurityError")
