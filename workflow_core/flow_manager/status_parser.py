...
class StatusParser:
    def __init__(
        self, start_path: Optional[Path] = None, config_root: Optional[Path] = None
    ):
    ...
    def _load_config(self) -> Dict[str, Any]:
        ...
        if template_path.exists():
            try:
                ...
                logging.getLogger("flow_manager").info(
                    f"Generated config from template at {config_path}"
                )
                return json.loads(content)
            except Exception as e:
                logging.getLogger("flow_manager").warning(
                    f"Failed to generate config from template: {e}"
                )
        ...
        raise RuntimeError(
            f"Flow Config missing at {config_path} and could not be generated from template {template_path}. System Halted."
        )
    ...
    def _find_root(self, start_path: Path) -> Path:
        ...
        raise FileNotFoundError(
            f"Repository root not found. Searched up from {start_path} looking for {markers}"
        )
    ...
    def _find_status_file(self) -> Optional[Path]:
        ...
        for p in self.root.rglob("status.md"):
            logger.warning(
                f"Status file not found in configured paths. Falling back to recursive search found: {p}"
            )
            return p
    ...
    def validate_structure(self) -> None:
                    ...
                    if mark == "/":
                        ...
                        if not prefix:
                            ...
                            if self.config.get("strict_mode", False):
                                raise StatusParsingError(
                                    f"Line {line_num}: Strict Mode enforced. Task must have a valid prefix (e.g. 'Impl.Feature: ...'). Found: '{name}'"
                                )
                            pass
        ...
        if len(active_tasks) > 1:
            raise StatusParsingError(
                f"Multiple active tasks found: {active_tasks}. context slicing requires single-task focus."
            )
    ...
    def _validate_prefix_defined(self, prefix: str, line_num: int):
        ...
        if not found:
            raise StatusParsingError(
                f"Line {line_num}: Unknown prefix '{prefix}'. Defined: {valid_prefixes}"
            )
    ...
    def _determine_workflow(self, prefix: Optional[str]) -> str:
        ...
        if not prefix:
            if self.config.get("strict_mode", False):
                raise StatusParsingError(
                    "Strict Mode: Task must have a prefix to determine workflow."
                )
        ...
        if strict:
            raise StatusParsingError(
                f"Strict Mode: Unknown prefix '{prefix}'. Defined: {list(prefixes.keys())}"
            )
