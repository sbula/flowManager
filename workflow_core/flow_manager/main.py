...
def main():
    ...
    parser.add_argument(
        "command",
        choices=["start", "resume", "reopen", "status", "reset", "validate"],
        nargs="?",
        default="start",
    )
    parser.add_argument("task_id", nargs="?", default=None)
    parser.add_argument(
        "--force-workflow", help="Override the detected workflow", default=None
    )
    ...
    try:
        ...
        if args.command == "validate":
            try:
                ...
                print(
                    f"   Active Task: {ctx.get('id', 'None')} - {ctx.get('name', 'None')}"
                )
        ...
        if args.force_workflow:
            ...
            workflow_name = determine_workflow(
                args.command, active_ctx, target_task_id, status_parser
            )
...
def determine_workflow(
    command: str, active_ctx: dict, task_id: str, parser: StatusParser
) -> str:
    """
