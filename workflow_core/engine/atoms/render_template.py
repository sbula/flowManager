...
def run(args: Dict[str, Any], context: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if template_name == "Expert_Panel_Dynamic":
        ...
        if not rendered_content:
            return {
                "status": "SKIPPED",
                "message": "No active experts found for this context.",
            }
        ...
        module_def = next(
            (m for m in factory.modules if m.get("Name") == template_name), None
        )
        if not module_def:
            return {
                "status": "FAILED",
                "message": f"Module {template_name} not found in Factory",
            }
