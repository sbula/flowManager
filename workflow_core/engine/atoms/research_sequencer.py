...
def run(args: Dict[str, Any], context: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not expert_set_name:
        return {
            "status": "FAILED",
            "message": "Research_Sequencer requires 'expert_set' argument.",
        }
    ...
    if not expert_roles:
        return {
            "status": "FAILED",
            "message": f"Expert Set '{expert_set_name}' not found or empty.",
        }
    ...
    for role in expert_roles:
        ...
        if not focus_file.exists():
            ...
            checklist = "\n".join(
                [f"   - {item}" for item in persona.get("Checklist", [])]
            )
    ...
    return {
        "status": "DONE",
        "message": f"All Focus Documents for {expert_set_name} are present.",
    }
