...
from workflow_core.core.template_factory.core import ReviewContext, TemplateFactory
...
def _resolve_experts(
    factory_modules: List[Dict[str, Any]],
    expert_set_name: Optional[str],
...
def run(args: Dict[str, Any], context: Dict[str, Any]) -> Dict[str, Any]:
    ...
    required_experts = _resolve_experts(
        all_active_modules, expert_set_name, teams_config, context
    )
    ...
    if last_expert_index == -1:
        ...
        return _build_waiting_msg(
            first_expert, personas_config, "Injected First Expert"
        )
    ...
    if "[x] APPROVE" not in segment:
        return _build_waiting_msg(
            last_expert, personas_config, f"Waiting for Approval from {last_role}"
        )
    ...
    if next_idx < len(required_experts):
        ...
        return _build_waiting_msg(
            next_expert,
            personas_config,
            f"Injected Next Expert: {next_expert.get('Role')}",
        )
