...
from ...flow_manager.template_factory.core import ReviewContext, TemplateFactory
...
class ReviewOrchestrator:
    ...
    def determine_next_action(
        ...
        if not report_path.exists():
            return self._initialize_report(
                task_id, report_path, expert_modules, base_modules, context
            )
        ...
        try:
            from ...flow_manager.scripts.validate_review_completeness import (
                check_review_completeness,
            )

            # Strict Mode: Check if current sections are filled
            if not check_review_completeness(
                str(report_path), ignore_pending=False, check_verdict=False
            ):
                return (
                    "BLOCKED",
                    f"Review Validation Failed: {report_path.name} is incomplete (Check Evidence/Status).",
                )
        except ImportError:
        ...
        if "| **Status** | [ ] PENDING |" in content:
            ...
            return (
                "BLOCKED",
                f"Expert Review Pending: {pending_role}. Please complete the report.",
            )
        ...
        if experts:
            ...
            if footer:
                content_parts.append(
                    self.factory.render_single_module(footer["Name"], ctx)
                )
    ...
    def _append_next_expert(self, path, content, expert, ctx) -> Tuple[str, str]:
        ...
        return (
            "BLOCKED",
            f"New Expert Unlocked: {expert['Role']}. Please perform review.",
        )
    ...
    def _collect_metrics(self, svc_path: Path, lang: str) -> Dict:
        ...
        try:
            from ...flow_manager.reconciler import (  # This import might be circular?
                _collect_test_metrics,
            )
            from ...flow_manager.templater import (
                format_complexity,
                format_lint,
                run_analysis,
            )
