...
class WorkflowEngine:
    ...
    def _export_context(
        ...
        for internal_key, context_key in export_map.items():
            if internal_key in output:
                ...
                logger.info(
                    f"Exporting Context: {context_key} = {val} (from {internal_key})"
                )
                context[context_key] = val

    def _resolve_instructions(
        self, instructions: Optional[str], context: Dict[str, Any]
    ) -> Optional[str]:
        """
    ...
    def run_workflow(
        self, task_id: str, workflow_name: str, context: Dict[str, Any] = None
    ) -> WorkflowState:
        """
        Main Entry Point.
        ...
        if not state:
            logger.info(
                f"Initializing new state for Task {task_id} (Workflow: {workflow_name})"
            )
            state = WorkflowState(
                task_id=task_id, workflow_ref=workflow_name, context_cache=context
            )
        else:
            # Verify Workflow Match (or Migration logic TODO)
            if state.workflow_ref != workflow_name:
                logger.warning(
                    f"Workflow mismatch! State has {state.workflow_ref}, requested {workflow_name}. Using State."
                )
    ...
    def _execute_steps(self, state: WorkflowState, workflow_def: WorkflowDefinition):
        ...
        while state.current_step_index < total_steps:
            ...
            # Execute
            logger.info(
                f"Executing Step: {step_id} ({step_def.type.value}: {step_def.ref})"
            )
            ...
            try:
                ...
                resolved_instructions = self._resolve_instructions(
                    step_def.instructions, state.context_cache
                )
                ...
                if step_def.type == StepType.ATOM:
                    ...
                    output = self.executor.execute_step(
                        step_def_runtime, state.context_cache
                    )
                    ...
                    if output.get("status") == "WAITING":
                        ...
                        logger.info(
                            f"Workflow Blocked at {step_id}: {output.get('message')}"
                        )
                    ...
                    if step_def.export:
                        self._export_context(
                            output, step_def.export, state.context_cache
                        )
                    ...
                    sub_state = self.run_workflow(
                        sub_task_id, step_def.ref, sub_context
                    )

                    if sub_state.current_step_index >= len(
                        self.loader.load_workflow(step_def.ref).steps
                    ):
