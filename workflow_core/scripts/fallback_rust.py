...
def scan_file_structure(content):
    ...
    rx_impl = re.compile(r'impl\s+(?:<.*?>\s*)?([a-zA-Z0-9_:]+)(?:\s+for\s+([a-zA-Z0-9_:]+))?') 
...
def scan_rust_files(root_dir):
    ...
    for dirpath, _, filenames in os.walk(root_dir):
        if "target" in dirpath.split(os.sep) or ".git" in dirpath or "legacy" in dirpath.split(os.sep):
            continue
                ...
                if msg.get("reason") == "compiler-message":
                    ...
                    if level in ["error", "warning"]:
                        ...
                        if spans:
                            ...
                            if file_name:
                                if file_name not in lint_results: lint_results[file_name] = []
                                lint_results[file_name].append(f"[{level.upper()}] {text}")
            except:
                pass
    except Exception as e:
