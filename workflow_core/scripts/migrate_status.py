...
def migrate_status(status_path):
    ...
    while i < len(lines):
        ...
        impl_match = re.search(r"(- \[[ x/]\]) (\d+(\.\d+)+)\. `Impl\.(Feature|Code)`: (.*)", line)
        ...
        if impl_match:
            ...
            if len(valid_lines) >= 1:
                ...
                if "Refactor" in line: gate_type = "Gate.Refactor"
                if "Fix" in line: gate_type = "Gate.Fix"
                
                # Construct New Line
                gate_line = f"{first_valid[:first_valid.find('-')]} {gate_status} {gate_id}. `{gate_type}`: Verify & Review."
                # Indentation preservation
                indent = line[:line.find("-")]
                gate_line = f"{indent}{gate_status} {gate_id}. `{gate_type}`: Verify & Review."
