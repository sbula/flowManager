...
def check_context_exists():
    ...
    if not status_path:
        ...
        phase_dirs = sorted([d for d in base.iterdir() if d.is_dir() and "phase" in d.name])
        if not phase_dirs:
    ...
    if status_path:
        ...
        phase_dir = cwd / "design" / "roadmap" / "phases" / "phase5" # Hardcoded fallback for reliability in this specific task
