...
class ContextManager:
    ...
    def _cleanup_resources(self, task_id: str, status_path: Path, artifact_dir: Path):
        ...
        root = self.config.root if hasattr(self.config, 'root') else Path.cwd() # Fallback
    ...
    def _resolve_artifact_dir(self, status_path: Path, task: Task) -> Path:
        ...
        try:
            ...
            candidates = [p for p in phase_dir.iterdir() if p.is_dir() and p.name.startswith(f"{root_id}_")]
